...
BEGIN
    ...
    -- 1️⃣ Verifica se existe usuário com email e senha
    SELECT COUNT(*) INTO user_count
    FROM TB_USER
    WHERE EMAIL = p_email AND PASS = p_pass;

    IF user_count > 0 THEN
        RETURN 'ok'; -- Login correto
    END IF;

    -- 2️⃣ Se não existe, verifica se o email existe
    SELECT COUNT(*) INTO user_count
    FROM TB_USER
    WHERE EMAIL = p_email;

    IF user_count = 0 THEN
        RETURN 'email'; -- Email incorreto
    ELSE
...
BEGIN
    ...
    -- Verifica se existe algum usuário com email e senha fornecidos
    IF EXISTS(
        SELECT 1 
        FROM TB_USER 
        WHERE EMAIL = p_email AND PASS = p_pass
